...
@section('content')
    <div class="card card-primary">
        <div class="card-header">
            <h3 class="card-title">Edit Detail Data</h3>
        </div>
        <div class="card-body">
            <form action="/detail10/{{$datadetail['id']}}" method="post" id="inputForm" enctype="multipart/form-data">
                @csrf
                <input type="hidden" id="id" name="id" value="{{$datadetail['id']}}">
                <div class="row">
                    <div class="col-md-6">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" id="tempat_lahir" name="tempat_lahir" class="form-control" placeholder="Tempat Lahir" value="{{old('tempat_lahir', $datadetail['tempat_lahir'])}}" required>
                            @error('tempat_lahir')
                        ...
                        <div class="form-group">
                            ...
                            <input type="date" id="tanggal_lahir" name="tanggal_lahir" class="form-control" placeholder="Tanggal Lahir" value="{{old('tanggal_lahir', $datadetail['tanggal_lahir'])}}" required>
                            @error('tanggal_lahir')
                                <div class="alert alert-danger">{{ $message }}</div>
                            @enderror
                        </div>
                        <div class="form-group">
                            ...
                            <select id="agama" name="agama" class="form-control custom-select" required>
                                ...
                                @foreach ($dataagama as $item)
                                    @if ($item['id'] == $datadetail['agama_id'])
                                        <option value="{{$item['id']}}" selected>{{$item['namaagama']}}</option>
                                    @else
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <textarea id="alamat" name="alamat" class="form-control" rows="4" placeholder="Alamat" required>{{old('alamat', $datadetail['alamat'])}}</textarea>
                            @error('alamat')
                                <div class="alert alert-danger">{{ $message }}</div>
                            @enderror
                        </div>
                        <div class="form-group">
                            ...
                            <div class="input-group">
                                <div class="custom-file">
                                    <input type="file" class="custom-file-input" id="foto_ktp" name="foto_ktp" value="{{$datadetail['foto_ktp']}}">
